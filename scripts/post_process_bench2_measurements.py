...
ALGORITHM_DUMSPTER_ROOT = os.path.dirname(os.path.abspath(__file__)) + '/..'
BENCH2_RESULTS = f'{ALGORITHM_DUMSPTER_ROOT}/data/bench'
OUPUT = f'{BENCH2_RESULTS}/all.json'

assert os.path.isdir(BENCH2_RESULTS)


def pathForGroup(group):
    return f"{BENCH2_RESULTS}/{group}"


def allGroups():
    return [x for x in os.listdir(BENCH2_RESULTS) if os.path.isdir(pathForGroup(x))]
